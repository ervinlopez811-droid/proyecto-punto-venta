...
public class PedidoService {
    ...
    public List<PedidoDTO> mostrarActivosOrdenTop5() {
        return pedidoRepository.findTop5ByEstadoTrueOrderByIdPedidoDesc()
                .stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }

    public List<PedidoDTO> filtroFecha(java.sql.Date fechaPedido) {
        return pedidoRepository.findByEstadoTrueAndFechaPedido(fechaPedido)
                .stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }
