...
public class PedidoDetalleService {
    ...
    public List<PedidoDetalleDTO> mostrarActivosOrdenTop5() {
        return pedidoDetalleRepository.findTop5ByEstadoTrueOrderByIdPedidoDetalleDesc()
                .stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }

    public List<PedidoDetalleDTO> filtroPedido(Integer idPedido) {
        Pedido pedido = new Pedido();
        pedido.setIdPedido(idPedido);
        return pedidoDetalleRepository.findByEstadoTrueAndIdPedido(pedido)
                .stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }
